...
-export([process_pic_simple/1,
	 process_pic_simple_limit/2, process_pic_simple_limit/3,
	 process_pic/2,
...
%% 对照片作简单的排序.
process_pic_simple({0, []}) ->
    {0, []};
process_pic_simple({Count, PicList}) ->
    %% 按照CreateDate来排序
...
process_pic_simple_limit(PicList, Len) ->
    process_pic_simple_limit(PicList, 1, Len).
...
%% {out_of_index, []}
%% {Count, Pic:list()}
process_pic_simple_limit({0, []}, _Start, _Len) ->
    {0, []};
process_pic_simple_limit({_Count, PicList}, Start, Len) ->
    %% 按照CreateDate来排序
    SortPicList = lists:sort(fun({pic, _Guid1, _Owner1, _Path1, _Type1, _Msg1, _Count1, _Dig1, _TagList1, _Spam1, CreateDate1},
				 {pic, _Guid2, _Owner2, _Path2, _Type2, _Msg2, _Count2, _Dig2, _TagList2, _Spam2, CreateDate2}) ->
			         case woomsg_datetime:compare_datetime(CreateDate1, CreateDate2) of
