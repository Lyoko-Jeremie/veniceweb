...
handle_get(Req) ->
    case woomsg_common:user_state(Req) of
        {login, Username} ->
	    Data = search_view:index(login, Username),
            Req:respond({200, [{"Content-Type","text/html"}], Data});
        {logout_remember, undefined} ->
	    Data = search_view:index(logout_remember, ?DEF_USERNAME),
            Req:respond({200, [{"Content-Type","text/html"}], Data});
        {logout_remember, Username} ->
	    Data = search_view:index(logout_remember, Username),
            Req:respond({200, [{"Content-Type","text/html"}], Data});
        {logout_no_remember, undefined} ->
	    Data = search_view:index(logout_no_remember, ?DEF_USERNAME),
            Req:respond({200, [{"Content-Type","text/html"}], Data})
    end.
