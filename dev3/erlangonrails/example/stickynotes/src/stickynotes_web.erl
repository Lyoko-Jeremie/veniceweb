...
loop(Req, _DocRoot) ->
	...
	{static, File} ->
	    "/" ++ StaticFile = File,
	    Req:serve_file(StaticFile, filename:join(stickynotes_deps:get_base_dir(), "priv/www"));
	{error,_} ->
...
render_template(ViewFile,Data,Env) -> 
    ...
    FullPathToFile = filename:join([stickynotes_deps:get_base_dir(), "src/views", ViewFile]),
